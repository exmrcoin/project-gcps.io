...
class ButtonMakerView(FormView):
    ...
    def form_valid(self, form):
        ...
        temp_html = ['<form action="https://'+domain+reverse('mtools:cryptopayV2') + '" method="POST" >',
                     ...
                     '<input type="image" src="https://'+str(domain+(ButtonImage.objects.get(
                         label=btn_image)).btn_img.url)+'"  alt="Buy Now with GetCryptoPayments.org" style="width: 200px;height: auto;"></form>'
                     ]
...
class DonationButtonMakerView(FormView):
    ...
    def form_valid(self, form):
        ...
        item_number = form.cleaned_data['item_number']
        allow_donator_to_adjust_amount = str(
            form.cleaned_data['allow_donator_to_adjust_amount']).lower()
        invoice_number = form.cleaned_data['invoice_number']
        tax_amount = form.cleaned_data['tax_amount']
...
class CryptoPaymment(FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        context['item_total'] = round(
            (float(item_qty) * float(item_amount)), 2)
...
class PaymentFormSubmitView(View):
    def post(self, request, *args, **kwargs):
        try:
            sel_coin = Coin.objects.get(
                code=self.request.POST['selected_coin'])
        except:
            sel_coin = EthereumToken.objects.get(
                contract_symbol=self.request.POST['selected_coin'])
...
class URLMakerInvoiceView(TemplateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['payable'] = (int(temp_obj.item_qty) * int(temp_obj.item_amount)
                              ) + temp_obj.shipping_cost + temp_obj.tax_amount
...
class POSCalcCoinSelect(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        domain = self.request.get_host()
        self.request.session['unique_transaction_id']= token
...
class POSCALCPayView(TemplateView):
...
class POSQRPayView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            context['amt_remaining'] = float(
                temp_obj.item_amount) - float(total_paid)
        except:
...
class CryptoPaymmentV2(FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        item_amount = float(item_obj.item_amount)
        tax_amount = float(item_obj.item_tax) * \
            float(self.request.POST['item_qty'])
...
class ButtonMakerInvoice(TemplateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context = super().get_context_data()
        if self.invoice_url:
            temp_obj = ButtonInvoice.objects.get(
                URL_link__icontains=self.invoice_url)
            #
            shipping_cost_add = 0
            #
            ...
            # attempt payment
            check_prepaid = MultiPayment.objects.filter(
                paid_unique_id=unique_id)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not self.invoice_url:
            ...
            try:
                self.invoice_url = domain + \
                    reverse('mtools:btnpay2', kwargs={
                            'token': self.invoice_url})
...
class DonationButtonMakerInvoice(TemplateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context = super().get_context_data()
        if self.invoice_url:
            temp_obj = DonationButtonInvoice.objects.get(
                URL_link__icontains=self.invoice_url)
            #
            shipping_cost_add = 0
            #
        ...
        if not self.invoice_url:
            ...
            try:
                self.invoice_url = domain + \
                    reverse('mtools:btnpay2', kwargs={
                            'token': self.invoice_url})
                temp_obj = SimpleButtonMakerInvoice.objects.get(
                    URL_link=self.invoice_url)
            except:
                pass
        #
...
class ButtonMakerPayView(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            merchant_id = (SimpleButtonInvoice.objects.filter(
                unique_id=unique_id))[0].merchant_id
        except:
            try:
                merchant_id = (ButtonInvoice.objects.filter(
                    unique_id=unique_id))[0].merchant_id
            except:
                try:
                    merchant_id = (DonationButtonInvoice.objects.filter(
                        unique_id=unique_id))[0].merchant_id
                except Exception as e:
                    raise e
        ...
        try:
            ...
            addr = create_wallet(Profile.objects.get(
                merchant_id=merchant_id).user, selected_coin, unique_id, True)
        ...
        time_expiry = MultiPayment.objects.filter(paid_unique_id=unique_id)[
            0].paid_date + timedelta(hours=8)
